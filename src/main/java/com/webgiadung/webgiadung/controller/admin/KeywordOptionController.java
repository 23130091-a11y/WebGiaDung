package com.webgiadung.doanweb.controller.admin;

import com.webgiadung.doanweb.model.Keywords;
import com.webgiadung.doanweb.services.KeywordService;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;

import java.io.IOException;
import java.io.PrintWriter;
import java.util.List;
import java.util.StringJoiner;

@WebServlet("/api/keywords")
public class KeywordOptionController extends HttpServlet {
    private KeywordService keywordService = new KeywordService();

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        response.setContentType("application/json");
        response.setCharacterEncoding("UTF-8");

        List<Keywords> list = keywordService.getAllKeywords();

        StringJoiner sj = new StringJoiner(",", "[", "]");

        for (Keywords k : list) {
            String name = (k.getName() != null) ? k.getName().replace("\"", "\\\"") : "";
            String item = String.format("{\"id\":%d,\"name\":\"%s\"}", k.getId(), name);
            sj.add(item);
        }

        PrintWriter out = response.getWriter();
        out.print(sj.toString());
        out.flush();
    }
}